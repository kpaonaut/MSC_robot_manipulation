% Modified by Wenjie Chen, 2016/07/27, FANUC Corporation
% Only 'start', 'stop', 'get' will be used in the experimentor package
...
function out = mouse3D_get(srv)
  ...
  % Added by Wenjie Chen, 2016/07/27. It seems either IsKeyUp or IsKeyDown could not be updated in real time.
%   out.btn1 = srv.Keyboard.IsKeyDown(1);     
%   out.btn2 = srv.Keyboard.IsKeyDown(2);
...
function [srv, tim] = mouse3D_start(period,timertag)
  ...
  tim = timerfind('tag',timertag);
% Commented out by Wenjie Chen, 2016/07/27. Timer is not necessary for experimentor package.
%   if isempty(tim),
%     tim = timer;
...
%   srv.invoke('Disconnect'); % This line does not seem to be needed
  ...
  delete(srv);  % Modified by Wenjie Chen, 2016/07/27
  srv = [];
  disp('Mouse3D stopped!');
