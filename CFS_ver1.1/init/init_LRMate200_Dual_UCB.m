...
ri{2}.Gripper2TCP = 0.200-0.027;        % Gripper with Adapter - Adapter
...
si.FS_ID_On = 1;    % 1- do force sensor calibration; 0- not do calibration
...
if si.FS_ID_On
    ...
    % with Handle Bar
%     ri{1}.ATI_Offset = [-10.2727, -3.63819, -9.28917, 0.12171, 0.54384, 0.14255];
%     ri{1}.PayloadG = [0, 0, -15.8803];                                          % Gravity force of payload in robot base frame
%     ri{1}.PayloadCG = [0.017042, -0.0042235, 0.067413];                         % CG of Payload in robot sensor frame 
% without Handle Bar
...
if si.FS_ID_On
    ...
    % with Handle Bar
%     ri{2}.ATI_Offset = [-12.5092, 5.52408, -0.564324, 0.044929, 0.39076, 0.12932];
%     ri{2}.PayloadG =  [0, 0, -17.9381];                                         % Gravity force of payload in robot base frame
%     ri{2}.PayloadCG = [0.025009, -0.0038461, 0.060038];                         % CG of Payload in robot sensor frame 
    % without Handle Bar
    ri{2}.ATI_Offset = [-5.88028, -8.81524, -16.1583, -0.29479, 0.53303, 0.14995];
    ri{2}.PayloadG =  [0, 0, -12.1349];                                         % Gravity force of payload in robot base frame
    ri{2}.PayloadCG = [-0.0053888, -0.00086675, 0.053853];                         % CG of Payload in robot sensor frame 
end
