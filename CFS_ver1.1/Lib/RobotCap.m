...
%% base
valpha=0.3; r=0.13; offset1=-0.329;offset2=0.25-0.329;
[X1,Y1,Z1]=sphere;
...
for k=1:md
for j=1:nd
    newvec=[X(k,j),Y(k,j),Z(k,j)]*M{1}(1:3,1:3)'+M{1}(1:3,4)';
    X(k,j)=newvec(1);
    Y(k,j)=newvec(2);
    Z(k,j)=newvec(3);
end
...
% alpha(boundary{i}.handle,valpha);
%% link4
r=0.08;offset1=0;offset2=0.350;
[X1,Y1,Z1]=sphere;
X=X1.*r;
Y=Z1.*r;
Z=Y1.*r;
...
for k=1:nd
for j=1:nd
    newvec=[X(k,j),Y(k,j),Z(k,j)]*M{5}(1:3,1:3)'+M{5}(1:3,4)';
    X(k,j)=newvec(1);
    Y(k,j)=newvec(2);
    Z(k,j)=newvec(3);
end
...
alpha(boundary{i}.handle,valpha);
%% link6
r=0.060;offset1=0.050;offset2=0.170;
[X1,Y1,Z1]=sphere;
X=Z1.*r;
Y=Y1.*r;
Z=X1.*r+0.135;
