...
for i = 1 : num    
    if opt.is_dual_arm  % currently dual arm uses LR Mate 200iD/7L robots
        ...
        [last_xyzwpr, i_cfg, ~] = rlfklr2d(last_jnt, rsi.utool, config_ref);    % last_jnt - rad;  utool - mm
    end
    ...
    if steps > 2 && ~opt.is_recursive   % only do one level recursion
        ...
        if opt.is_dual_arm  % currently dual arm uses LR Mate 200iD/7L robots
            ...
            [JntPos(i,:), ~] = rliklr2d(xyzwpr(i,:), i_cfg, rsi.utool, last_jnt, choice);  % JntPos - rad;  xyzwpr - mm, deg;  last_jnt - rad;  utool - mm
        end        
    end
    last_jnt = JntPos(i,:);
end
