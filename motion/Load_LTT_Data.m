...
%  Modified by Te Tang, 09/28/2016, add opt.FileName to load data from specified file 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function LTT_Data = Load_LTT_Data(si, opt)
...
if ~isfield(opt,'DataMode') || isempty(opt.DataMode),  opt.DataMode = 1;  end   % 0 - use previous LTT data to replay (default); 1 - get new LTT data but no replay
...
if ~isfield(opt,'FileName') || isempty(opt.FileName),  opt.FileName = 0;  end   % 0 - no specify file; 'filename str'
...
if opt.FileName ~= 0
    disp('---> Load Teaching Data.')
    LTTData = load([opt.relPath2Data, opt.FileName ]);
else
    if opt.DataMode == 1
...
for rn = 1:numel(rsi)
    ...
    LTT_Data.DesJntPos{rn} = DesJntPos;
    LTT_Data.GrpCmd{rn} = GrpCmd;
    LTT_Data.ReplayTime{rn} = ReplayTime;
